...
public class ZigBeeNetworkManagerImpl implements ZigBeeNetworkManager
{
    ...
    private ObservableState<DriverStatus> driverStatus = new ObservableState<>(DriverStatus.CLOSED);
    ...
    public ZigBeeNetworkManagerImpl(ZigBeePort port, NetworkMode mode, int pan, int channel, long timeout) {
        ...
        final StateChangeObserver<DriverStatus> stateObserver = new StateChangeObserver<DriverStatus>()
        {
            private DriverStatus oldStatus;

            @Override
            public void onStateChanged(final DriverStatus newStatus)
            {
    ...
    public void startup() {
        if (driverStatus.is(DriverStatus.CLOSED)) {
            ...
            if (!initializeZigBeeNetwork(false)) {
                shutdown();
            }

            // Now reset the dongle
                driverStatus.set(DriverStatus.HARDWARE_OPEN);
            if (!dongleReset()) {
